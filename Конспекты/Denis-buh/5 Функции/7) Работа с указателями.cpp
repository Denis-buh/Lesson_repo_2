...
arrey sum(arrey *arr, const int LEN_arr){
    // *arr - указатель для работы с массивом (так как без указателя получем 1 элемент масива)
    arrey res = 0;
...
void my_fun(int *number, int *number_1){// Не использовать из-за возможных непоняток в коде. Данный метод позволяет возвращать и менять сразу несколько переменных из функции
    // *number * необходима для того чтобы получить адрес 
    int res = *number  + 1; // Разъименовываем указатель (получам значение) и прибавляем к нему 1
