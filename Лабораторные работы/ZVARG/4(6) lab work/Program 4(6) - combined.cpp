...
int min_finding_with_pointer(int* arr, int arr_dim);//функция по нахождению минимального из массива (с ук-лями)
int arr_view(int arr_dim, int arr[]);// функция для вывода массива (без ук-лей)
int arr_view_with_pointer(int* arr, int arr_dim);// функция для вывода массива (с ук-лями)
int number_of_neg_el(int arr[], int arr_dim);//функция, выполняющая подсчёт эл-ов для отдельной строки (без ук-лей)
int number_of_neg_el_with_pointer(int* arr, int arr_dim);//функция, выполняющая подсчёт эл-ов для отдельной строки (с ук-лями)
...
int main()
{
	...
	bool program_execution_variant;
	...
	cout << "Задайте вариант выполнения программы: 0 для выполнения программы с помощью алгоритмов с указателями" << endl;
	cout << "А 1 - для выполнения программы с помощью алгоритмов, построенных 'стандартным' способом" << endl;
	cin >> program_execution_variant;
	if (program_execution_variant == 0) 
	{
		int sum;
		for (int layer = 0; layer < arr_dim_numb; layer++)//выполнение программы с ук-лями
		{
			sum+=number_of_neg_el_with_pointer(main_array[layer], arr_dim_numb);
		}
		cout << "Сумма:" << sum << endl;
	}
	else
...
int min_finding_with_pointer(int* arr, int arr_dim)//функция по нахождению минимального из массива (с ук-лем)
{
	int min(*arr);
	for (int el_index = 0; el_index < arr_dim; el_index++)
	{
...
int arr_view_with_pointer(int* arr, int arr_dim) // функция для вывода массива (с ук-лем)
{
	cout << " array: " << endl;
	for (int el_index = 0; el_index < arr_dim; el_index++)
	{
...
int number_of_neg_el_with_pointer(int* arr, int arr_dim) //функция, выполняющая подсчёт эл-ов для отдельной строки (с ук-лями)
{
	int sum;
	int min;
	min = min_finding_with_pointer(arr, arr_dim);
	for (int el_index = 0; el_index < arr_dim; el_index++)
	{
		if (*(arr + el_index) == min) break;
		if (*(arr + el_index) < 0) sum += *(arr + el_index);
